...
function wrapFunctions(...)
   ...
   for _,value in ipairs(arg) do storeAsMethod(value) end
	return testClass
end
...
UnitResult = { -- class
	...
	function UnitResult:displayFailedTests()
		...
		print("Failed tests:")
		print("-------------")
      for _,value in ipairs(self.errorList) do self:displayOneFailedTest(value) end
		print()
	end
...
LuaUnit = {
	...
	function LuaUnit:run(...)
		...
		if arg.n > 0 then
         for _,value in ipairs(arg) do LuaUnit:runTestClassByName(value) end         
		else 
			if argv and argv.n > 0 then
            for _,value in ipairs(argv) do LuaUnit:runTestClassByName(value) end
			else
